...
1. Go to https://github.com/new and create a new **private** repository
   (e.g. `capstone-sharedshoppinglist`). Don't initialize it with a README —
   this project already has one.
2. Back in your terminal:
   ```bash
   git remote add origin https://github.com/<your-username>/capstone-sharedshoppinglist.git
   git branch -M main
   git push -u origin main
   ```
3. If prompted for credentials, use a
   [personal access token](https://github.com/settings/tokens) instead of
   your password (GitHub no longer accepts password auth over HTTPS).
...
7. **Network settings → Edit**:
   ...
   - Allow: SSH (22) from **My IP** (not `0.0.0.0/0` — no reason to expose
...
3. Note this IP — it's what you'll SSH to and point DNS at.
...
```bash
git clone https://github.com/<your-username>/capstone-sharedshoppinglist.git
cd capstone-sharedshoppinglist
```
...
- Check spend anytime: **AWS Console → Billing → Cost Explorer**, or set a
  ...
  say, $20 so you get an email well before the $100 ceiling.
